    ...
    while (running) {
        ...
        if (viewMode_ == ViewMode::Game) {
            ...
            if (isBattleMode(session.mode)) {
                if (session.finished && !session.remoteFinished) {
                    statusMessage_ = "你已完成，等待对手结束后统一结算。";
                } else if (!session.finished && session.remoteFinished) {
                    statusMessage_ = "对手已完成，请尽快结束本局。";
                } else if (session.finished && session.remoteFinished && !session.battleWinnerText.empty()) {
                    statusMessage_ = session.battleWinnerText;
                }
            }
...
void IdiomChainScene::refreshBattleSetupButtons() {
    ...
    battleHostButton_ = ButtonSpec{makeRect(x, 250, w, h), "创建房间", 1, true};
    battleJoinButton_ = ButtonSpec{makeRect(x, 338, w, h), "加入房间", 2, true};
    ...
    battleConfirmButton_ = ButtonSpec{makeRect(x, 624, w, 50),
                                      ...
                                      6, true};
    battleSetupBackButton_ = ButtonSpec{makeRect(x, 686, w, 50), "返回主菜单", 7, true};
}
...
void IdiomChainScene::renderGame() {
    ...
    if (isBattleMode(session.mode)) {
        explanationText += battleRemoteSummary();
        explanationText += "\n\n";
    }
...
void IdiomChainScene::renderResult() {
    ...
    drawPanel(centerPanel, isBattleMode(session.mode) ? "双方对比" : "路径对照");
    ...
    if (isBattleMode(session.mode)) {
        ...
        resultMyRouteContentHeight_ = drawWrappedTextScrollable(
            smallFont_, battleResultSummary(),
        ...
        fillRect(renderer_, pathCard, rgb(255, 255, 255));
        strokeRect(renderer_, pathCard, rgb(36, 36, 36), 1);
        drawFittedTextInRect(bodyFont_, "路径对照",
...
std::string IdiomChainScene::battleRemoteSummary() const {
    const GameSession& session = controller_.getSession();
    std::ostringstream oss;
    oss << "对手：" << (session.remotePlayerName.empty() ? "等待中" : session.remotePlayerName) << "\n";
    oss << "对手步数：" << session.remoteStepCount << "\n";
    oss << "对手用时：" << formatSeconds(session.remoteElapsedSeconds) << "\n";
    oss << "对手得分：" << session.remoteScore << "\n";
    oss << "对手状态：";
    if (session.remoteFinished) {
        oss << "已完成";
    } else if (controller_.isBattleConnected()) {
        oss << "进行中";
    } else {
        oss << "未连接";
    }
...
std::string IdiomChainScene::battleResultSummary() const {
    const GameSession& session = controller_.getSession();
    std::ostringstream oss;
    oss << "对手：" << (session.remotePlayerName.empty() ? "对手" : session.remotePlayerName) << "\n";
    oss << "用时：" << formatSeconds(session.remoteElapsedSeconds) << "\n";
    oss << "步数：" << session.remoteStepCount << "\n";
    oss << "得分：" << session.remoteScore << "\n";
    oss << "状态：" << (session.remoteFinished ? "已完成" : "未完成");
    return oss.str();
}
