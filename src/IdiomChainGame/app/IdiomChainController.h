...
// private:
//     std::string evaluateStepQuality(int fromId, int toId) const;
...
private:
    int pendingHintId_ { -1 };
    int pendingHintFromId_ { -1 };
    bool pendingHintNeedReveal_ { false };
    int pendingHardHintId_ { -1 };
    int pendingHardHintFromId_ { -1 };
    bool pendingHardHintNeedReveal_ { false };
    bool canReachTargetAvoidingUsed(int fromId) const;
    bool hasAnyReachableNextFromCurrent() const;
    void prepareSession(IModeStrategy& strategy);
    void prepareFixedBattleSession(GameMode mode, int startId, int targetId);
    void initializeSessionForQuestion(GameMode mode, int startId, int targetId, bool generateEasyPool);
    void updateElapsedSeconds();
