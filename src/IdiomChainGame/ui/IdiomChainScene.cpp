...
//         titleFont_ = TTF_OpenFont(path.c_str(), 44);
//         subtitleFont_ = TTF_OpenFont(path.c_str(), 28);
//         bodyFont_ = TTF_OpenFont(path.c_str(), 22);
//         smallFont_ = TTF_OpenFont(path.c_str(), 18);
...
//     if (event.type == SDL_TEXTINPUT && activeModeIndex_ == 3) {
...
//             revealedAnswerText_ = joinWords(answerPathWords(), " -> ");
//             statusMessage_ = "已显示一条最优解。";
//             return;
//         }
...
//     easyTiles_.clear();
//     easySelectedZoneRect_ = makeRect(344, 346, 512, 160);
//     easyDiscardZoneRect_ = makeRect(344, 522, 512, 160);
...
//                                  zone.y + 44 + row * (kTileHeight + kTileGap),
//                                  kTileWidth,
//                                  kTileHeight);
...
//     strokeRect(renderer_, button.rect, rgb(30, 30, 30), 2);
//     drawText(subtitleFont_, button.label,
...
//     drawText(bodyFont_, title, rect.x + 12, rect.y + 10, rgb(22, 22, 22), false);
//     drawWrappedText(smallFont_, content, makeRect(rect.x + 12, rect.y + 36, rect.w - 24, rect.h - 42), rgb(92, 92, 92));
// }
...
//         }
//         if (i < columns.size()) {
//             drawText(header ? bodyFont_ : smallFont_, columns[i], x + widths[i] / 2, rect.y + rect.h / 2, rgb(20, 20, 20), true);
//         }
//         x += widths[i];
//     }
// }
...
//     drawText(bodyFont_, "当前用时：" + formatSeconds(session.elapsedSeconds), timeBox.x + 14, timeBox.y + 14, rgb(20, 20, 20), false);
...
//     drawCard(makeRect(leftPanel.x + 18, leftPanel.y + 214, leftPanel.w - 36, 76),
//              "题目",
...
//     drawCard(makeRect(leftPanel.x + 18, leftPanel.y + 304, leftPanel.w - 36, 96),
//              "当前状态",
...
//     drawCard(makeRect(leftPanel.x + 18, leftPanel.y + 414, leftPanel.w - 36, 72),
//              "补充信息",
//              helper.str());
...
//     drawText(bodyFont_, "起点：" + controller_.wordOf(session.startId), startBox.x + 12, startBox.y + 18, rgb(20, 20, 20), false);
//     drawText(bodyFont_, "终点：" + controller_.wordOf(session.targetId), targetBox.x + 12, targetBox.y + 18, rgb(20, 20, 20), false);
...
//     drawCard(makeRect(centerPanel.x + 24, centerPanel.y + 122, centerPanel.w - 48, 72),
//              "当前路径链",
//              currentPathText().empty() ? "暂无路径。" : currentPathText());
...
//         drawText(bodyFont_, "路径区：把正确路线拖到这里", easySelectedZoneRect_.x + 14, easySelectedZoneRect_.y + 10, rgb(40, 96, 54), false);
//         drawText(bodyFont_, "候选池 / 干扰区", easyDiscardZoneRect_.x + 14, easyDiscardZoneRect_.y + 10, rgb(112, 80, 54), false);
...
//         fillRect(renderer_, inputBox, rgb(255, 255, 255));
//         strokeRect(renderer_, inputBox, rgb(36, 36, 36), 1);
//         drawText(bodyFont_, inputBuffer_.empty() ? "请输入下一个成语，或输入缩写 / 拼音片段" : inputBuffer_,
//                  inputBox.x + 12, inputBox.y + 15,
//                  inputBuffer_.empty() ? rgb(130, 130, 130) : rgb(20, 20, 20), false);
//         hardSubmitButton_ = ButtonSpec{makeRect(centerPanel.x + centerPanel.w - 164, centerPanel.y + 368, 140, 44), "提交输入", 1, true};
//         drawButton(hardSubmitButton_, true);

//         drawText(bodyFont_, "匹配候选（点击可直接提交）", centerPanel.x + 24, centerPanel.y + 428, rgb(70, 70, 70), false);
...
//         explanationText += "\n\n最优解：\n" + revealedAnswerText_;
//     }
//     drawWrappedText(bodyFont_, explanationText,
...
//     drawCard(makeRect(centerPanel.x + 18, centerPanel.y + 54, centerPanel.w - 36, 128),
//              "你的路线",
//              currentPathText().empty() ? "暂无记录。" : currentPathText());
//     drawCard(makeRect(centerPanel.x + 18, centerPanel.y + 198, centerPanel.w - 36, 170),
//              "最优路线",
...
//     int rowY = header.y + 58;
//     for (std::size_t i = 0; i < leaderboardCache_.size() && i < 8; ++i) {
...
//     int hy = historyPanel.y + 56;
//     if (historyCache_.empty()) {
//         drawWrappedText(bodyFont_, "暂无记录。完成至少一局后这里会显示最近的对局信息。",
//                         makeRect(historyPanel.x + 16, hy, historyPanel.w - 32, 120), rgb(82, 82, 82));
//     } else {
//         for (std::size_t i = 0; i < historyCache_.size() && i < 6; ++i) {
